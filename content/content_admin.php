	...
	else if($_GET['action'] == "1")
	{
		...
		else
		{
			if(isset($_GET['user']))
			{
				...
				$query = "SELECT u.user_id, u.user_name, u.user_image, u.user_acceslevel, k.klant_id, k.klant_voornaam, k.klant_achternaam, k.klant_email
					FROM klanten k
				...
				while ($row = mysql_fetch_array($resultaat)) 
				{
					...
					<form action="changeitem.php" method="POST" enctype="multipart/form-data">
						<table border="0px;">
							...
							<input type="hidden" name="returnpage" value="admin&action=1&state=done"/>
			...
			else
			{
				...
				?>
						<td style="width:70px;"><a href="index.php?page=admin&action=1&user= <?php echo $row['user_id']; ?>"><img style="width:60px;height:60px;border:1px solid lightblue;" alt="Can\'t load image" src="<?php echo $image; ?>"/></a></td>
						<td style="width:150px;"><a href="index.php?page=admin&action=1&user= <?php echo $row['user_id']; ?>" style="text-decoration:none;"><span style="font-size:15px;font-weight:bold;"><?php echo $row['user_name']; ?></span><a/></td>
						<td style="width:150px;"><a href="index.php?page=admin&action=1&user= <?php echo $row['user_id']; ?>" style="text-decoration:none;"><span style="font-size:15px;font-weight:bold;"><?php echo $row['klant_voornaam']; ?></span><a/></td>
						<td style="width:150px;"><a href="index.php?page=admin&action=1&user= <?php echo $row['user_id']; ?>" style="text-decoration:none;"><span style="font-size:15px;font-weight:bold;"><?php echo $row['klant_achternaam']; ?></span><a/></td>
					</tr>
				<?php 
					$t++;
				}
